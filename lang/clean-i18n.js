...
function phraseKeys(lang,d){
 ...
 return phraseCache[lang]=Object.keys(d).filter(k=>k&&d[k]!==k&&k.length>=4&&/[A-Za-z]/.test(k)&&!/^S\d\b/.test(k)).sort((a,b)=>b.length-a.length);
}
...
function canonicalText(n){
 ...
 if(!textBase.has(n)||last===undefined||now!==last)textBase.set(n,now);
 return textBase.get(n);
}
