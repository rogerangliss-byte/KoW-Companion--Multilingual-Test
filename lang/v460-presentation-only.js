...
function tr(s,lang=active){if(lang==='en'||!s)return s;const d=D()[lang]||{};if(Object.prototype.hasOwnProperty.call(d,s))return d[s];let o=s;for(const [rx,rp] of Q12[lang]||[]){if(rx.test(o)){o=o.replace(rx,rp);break}}if(o!==s)return o;for(const [rx,rp] of R[lang]||[]){if(rx.test(o)){o=o.replace(rx,rp);break}}if(o!==s)return o;for(const k of Object.keys(d).filter(k=>k.length>=5&&o.includes(k)).sort((a,b)=>b.length-a.length)){o=o.split(k).join(d[k])}return o}
