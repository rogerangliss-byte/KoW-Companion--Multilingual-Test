/* KoW Companion v4.6.0 — Language QA13 ROBUST English-bleed + coverage audit. */
(function(){
'use strict';
...
function renderedSnapshot(){
 ...
 for(const id of ROOTS){
   if(id==='settings'){ /* QA13: QA panel diagnostics are audited separately, not as app translation */ }
...
function build(){
 ...
 return{
   ...
   qa_version:'Language QA13 ROBUST BLEED',
...
function download(rep){const b=new Blob([JSON.stringify(rep,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`KoW-Language-QA13-v4.6.0-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
function render(rep){
 ...
 if(status)status.textContent='QA13 complete.';
 if(results){
 ...
 if(dl){
   dl.disabled=false;
   dl.onclick=()=>download(window.KOW_LAST_LANGUAGE_QA13);
 }
}
...
function showQaError(err){
 const status=document.getElementById('languageQaStatus');
 const results=document.getElementById('languageQaResults');
 if(status)status.textContent='QA13 FAILED TO RUN — see details below.';
 if(results)results.innerHTML=`<div class="notice"><b>QA runner error:</b> ${String(err&&err.message||err)}</div>`;
 console.error('KoW Language QA13 runner error',err);
}
...
function run(){
 ...
 if(status)status.textContent='Running QA13 across EN / FR / DE / IT…';
 ...
 setTimeout(()=>{
   try{
     const rep=build();
     window.KOW_LAST_LANGUAGE_QA13=rep;
     render(rep);
   }catch(err){
     showQaError(err);
   }finally{
...
function wire(){
 ...
 p.innerHTML=
   '<h3>🧪 Language QA13 / Read-Only Dynamic-Language Proof</h3>'+
   ...
   '<div class="two">'+
     '<button id="runLanguageQa" class="app-action-primary" type="button">Run Language QA13</button>'+
     '<button id="downloadLanguageQaReport" class="app-action-secondary" type="button" disabled>↓ Download QA13 Report</button>'+
   '</div>'+
   '<div id="languageQaStatus" class="notice" style="margin-top:10px">QA13 ready.</div>'+
...
window.KOW_LANGUAGE_QA13={
 run,
 report:()=>window.KOW_LAST_LANGUAGE_QA13||null,
 version:'QA13'
};
})();