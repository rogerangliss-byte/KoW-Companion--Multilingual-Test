/* KoW Companion v4.6.0 — Language QA15 ROBUST English-bleed + coverage audit. */
(function(){
'use strict';
...
function renderedSnapshot(){
 ...
 for(const id of ROOTS){
   if(id==='settings'){ /* QA15: QA panel diagnostics are audited separately, not as app translation */ }
...
function build(){
 ...
 return{
   ...
   qa_version:'Language QA15 ROBUST BLEED',
...
function download(rep){const b=new Blob([JSON.stringify(rep,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`KoW-Language-QA15-v4.6.0-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
function render(rep){
 ...
 if(status)status.textContent='QA15 complete.';
 if(results){
 ...
 if(dl){
   dl.disabled=false;
   dl.onclick=()=>download(window.KOW_LAST_LANGUAGE_QA15);
 }
}
...
function showQaError(err){
 const status=document.getElementById('languageQaStatus');
 const results=document.getElementById('languageQaResults');
 if(status)status.textContent='QA15 FAILED TO RUN — see details below.';
 if(results)results.innerHTML=`<div class="notice"><b>QA runner error:</b> ${String(err&&err.message||err)}</div>`;
 console.error('KoW Language QA15 runner error',err);
}
...
function run(){
 ...
 if(status)status.textContent='Running QA15 across EN / FR / DE / IT…';
 ...
 setTimeout(()=>{
   try{
     const rep=build();
     window.KOW_LAST_LANGUAGE_QA15=rep;
     render(rep);
   }catch(err){
     showQaError(err);
   }finally{
...
function wire(){
 ...
 p.innerHTML=
   '<h3>🧪 Language QA15 / Read-Only Dynamic-Language Proof</h3>'+
   ...
   '<div class="two">'+
     '<button id="runLanguageQa" class="app-action-primary" type="button">Run Language QA15</button>'+
     '<button id="downloadLanguageQaReport" class="app-action-secondary" type="button" disabled>↓ Download QA15 Report</button>'+
   '</div>'+
   '<div id="languageQaStatus" class="notice" style="margin-top:10px">QA15 ready.</div>'+
...
window.KOW_LANGUAGE_QA15={
 run,
 report:()=>window.KOW_LAST_LANGUAGE_QA15||null,
 version:'QA15'
};
})();