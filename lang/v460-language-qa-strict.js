/* KoW Companion v4.6.0 — Language QA14 ROBUST English-bleed + coverage audit. */
(function(){
'use strict';
...
function renderedSnapshot(){
 ...
 for(const id of ROOTS){
   if(id==='settings'){ /* QA14: QA panel diagnostics are audited separately, not as app translation */ }
...
function build(){
 ...
 return{
   ...
   qa_version:'Language QA14 ROBUST BLEED',
...
function render(rep){
 ...
 if(status)status.textContent='QA14 complete.';
 if(results){
 ...
 if(dl){
   dl.disabled=false;
   dl.onclick=()=>download(window.KOW_LAST_LANGUAGE_QA14);
 }
}
...
function showQaError(err){
 const status=document.getElementById('languageQaStatus');
 const results=document.getElementById('languageQaResults');
 if(status)status.textContent='QA14 FAILED TO RUN — see details below.';
 if(results)results.innerHTML=`<div class="notice"><b>QA runner error:</b> ${String(err&&err.message||err)}</div>`;
 console.error('KoW Language QA14 runner error',err);
}
...
function run(){
 ...
 if(status)status.textContent='Running QA14 across EN / FR / DE / IT…';
 ...
 setTimeout(()=>{
   try{
     const rep=build();
     window.KOW_LAST_LANGUAGE_QA14=rep;
     render(rep);
   }catch(err){
     showQaError(err);
   }finally{
...
function wire(){
 ...
 p.innerHTML=
   '<h3>🧪 Language QA14 / Read-Only Dynamic-Language Proof</h3>'+
   ...
   '<div class="two">'+
     '<button id="runLanguageQa" class="app-action-primary" type="button">Run Language QA14</button>'+
     '<button id="downloadLanguageQaReport" class="app-action-secondary" type="button" disabled>↓ Download QA14 Report</button>'+
   '</div>'+
   '<div id="languageQaStatus" class="notice" style="margin-top:10px">QA14 ready.</div>'+
...
window.KOW_LANGUAGE_QA14={
 run,
 report:()=>window.KOW_LAST_LANGUAGE_QA14||null,
 version:'QA14'
};
})();