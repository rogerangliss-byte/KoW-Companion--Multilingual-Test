/* KoW Companion v4.6.0 CLEAN — comprehensive rendered-language QA */
(function(){
'use strict';
...
function isEnglish(t){if(PHRASES.some(p=>t.toLowerCase().includes(p.toLowerCase())))return true;const words=(t.replace(/\bS\d\b/gi,' ').match(/[A-Za-z][A-Za-z'-]*/g)||[]).map(x=>x.toLowerCase());let m=0,h=0;for(const w of words){if(TECH.has(w)||w.length<3)continue;m++;if(ENWORDS.has(w))h++}return h>=3 && h/Math.max(1,m)>=0.45}
function bleed(code){if(code==='en')return[];return [...new Set(nodes().filter(x=>isEnglish(x.t)).map(x=>`${x.id}: ${x.t}`))]}
